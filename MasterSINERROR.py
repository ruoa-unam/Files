...
def merge_files(folder_path, include = [''], exclude = ':', skip_rows = []):
    ...
    for s in onlyfiles:
        if any(i.lower() in s.lower() for i in include) and exclude.lower() not in s.lower():
            ...
            data = pd.read_csv(route,sep=',', encoding='latin1', skiprows = skip_rows, na_values = ['NAN','null','nan',' '], quoting=3, error_bad_lines=False)
            # appending the data
...
def save_month(folder_path, site, df, version = 'L0', extension = '.csv', na_representation = 'null', Timestamp = False, Record = False, MDR = True, sheet_name = 'met', area = 'Meteorological'):
    ...
    for group in grouped_year_month:
        ...
        round(Gdf, 3).to_csv(join(folder_path, filename), na_rep = na_representation,  index=False, encoding = 'latin1')   
        ### header
...
def df_mean(df, site, rule = '60Min', sheet_name = 'met'):
    ...
    try:
        ...
        df1 = pd.read_csv(route1, sep=',',skiprows=[0,1,2,3,4,5,7], encoding='latin1', na_values=['null','NAN','nan'], error_bad_lines=False)     
...
def reworkdates(site, task, begin, counter, version = 'L0'): 
    ...
    if counter == 1:
        ...
        #-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        # read the file
        R = pd.read_csv(route1, error_bad_lines=False)
        # set datetime64 into the 'Last Updload Date' column
    ...
    if not np.all(B):
        ...
        for f in sorted(onlyfiles):
            ...
            for (Y, M), G in grouped:
                ...
                data = pd.read_csv(route2, sep=',',skiprows=[0,1,2,3,4,5,7],encoding='latin1',na_values=['null','NAN'], error_bad_lines=False,quotechar=none, quoting=3) 
