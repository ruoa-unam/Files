...
def merge_files(folder_path, include = [''], exclude = ':', skip_rows = []):
    ...
    for s in onlyfiles:
        if any(i.lower() in s.lower() for i in include) and exclude.lower() not in s.lower():
            ...
            data = pd.read_csv(route,sep=',', encoding='latin1', skiprows = skip_rows, na_values = ['NAN','null','nan',' '], error_bad_lines=False)
            # appending the data
    ...
    if not np.all(B):
        ...
        for f in sorted(onlyfiles):
            ...
            for (Y, M), G in grouped:
                ...
                data = pd.read_csv(route2, sep=',',skiprows=[0,1,2,3,4,5,7],encoding='latin1',na_values=['null','NAN'], error_bad_lines=False) 
